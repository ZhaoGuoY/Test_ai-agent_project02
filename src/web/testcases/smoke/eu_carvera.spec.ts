// EU 站点 — Add to cart → Check Out 结算流程冒烟测试
...
test.describe('EU 站点', () => {
  ...
  test('商品加入购物车', async ({ sharedPage: page }) => {
    ...
    await test.step('页面初始化与跳转防护', async () => {
      ...
      if (currentHostBeforeAssert !== targetHostForCheck || !hasProductPath) {
        console.warn(`[EU] ️ setupPage 后检测到延迟重定向: ${currentUrlBeforeAssert} → 重新执行 setupPage 纠正（最多 1 次）`);
    ...
    await test.step('弹窗清理并点击 Add to cart 按钮', async () => {
      console.log(`[EU] ✅ Add to cart 按钮可见，准备点击加购`);
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        try {
          ...
          console.warn(`[EU] ⚠️ 第${attempt}轮点击失败（可能被浮窗遮挡），重新清理后重试`);
          if (attempt === 3) throw new Error('[EU] Add to cart 按钮 3 轮点击均失败');
        }
      }
      console.log(`[EU] 🛒 已点击 Add to cart，等待购物车抽屉弹出...`);
    });
  ...
  test('商品正常进入结算页', async ({ sharedPage: page }) => {
    ...
    await test.step('弹窗清理并点击 Check out 按钮', async () => {
      ...
      console.log(`[EU] ✅ Check Out 按钮可见，准备进入结算页`);
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        try {
          ...
          console.warn(`[EU] ⚠️ 第${attempt}轮点击失败（可能被浮窗遮挡），重新清理后重试`);
          if (attempt === 3) throw new Error('[EU] Check out 按钮 3 轮点击均失败');
        }
      }
      console.log(`[EU] 💳 已点击 Check Out，等待结算页加载...`);
    });
    ...
    await test.step('断言结算页商品存在且 URL 跳转成功', async () => {
      ...
      console.log(`[EU] ✅ 已跳转至结算页: ${page.url()}`);
    });
  });
